    ...
    for indicator_code in pack.indicators:
        if country_code == "us" and indicator_code == "gdp":
            ...
            try:
                ...
                diagnostic_dir = model_input_path.parent
                ...
                payload = _payload_from_model_run(
                    ...
                    extra_downloads=US_GDP_DIAGNOSTIC_FILES,
                )
        ...
        if country_code == "us" and indicator_code == "gdp" and diagnostic_dir is not None:
            _copy_optional_artifacts(indicator_dir, diagnostic_dir, US_GDP_DIAGNOSTIC_FILES)
...
def _payload_from_model_run(
    ...
    *,
    ...
    return {
        "metadata": _metadata(pack, meta, latest.reference_period, methodology=methodology, extra_downloads=extra_downloads),
        "latest": {
            "schema_version": SCHEMA_VERSION,
...
def _copy_optional_artifacts(indicator_dir: Path, source_dir: Path, files: tuple[str, ...]) -> None:
    for file in files:
        source = source_dir / file
        if source.exists():
            copyfile(source, indicator_dir / file)
